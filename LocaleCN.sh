...
if [ "${release}" == "centos" ]; then
	...
	cd /etc
	rm -rf locale.conf
	wget https://raw.githubusercontent.com/hczjl/Note/main/locale.conf > /dev/null 2>&1
	...
	cd /etc/
	wget https://raw.githubusercontent.com/hczjl/Note/main/locale.gen > /dev/null 2>&1
	locale-gen
	cd /etc/default/
	wget https://raw.githubusercontent.com/hczjl/Note/main/locale.conf > /dev/null 2>&1
	cp locale.conf locale
